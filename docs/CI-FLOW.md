...
## infra 자신의 검사 (`lint-workflows.yml`)

이 레포에 있는 `ci-*.yml` / `e2e-*.yml` 은 전부 `workflow_call` 전용이라
**자기 레포에서는 절대 실행되지 않습니다.** 즉 infra 자체에는 원래 게이트가
없었습니다. 문법이 깨진 채로 main 에 들어가면, 세 서비스 레포가 `@main` 으로
참조하는 순간 즉시 함께 깨지고 — 재사용 워크플로 오류는 job 0개·로그 0개로
조용히 죽어서 원인 파악이 어렵습니다.

```
PR / main push (이 레포 자신)
   └─ actionlint 를 공식 설치 스크립트로 받아 .github/workflows/*.yml 검사
```

이건 PR·push 로 자동 도는 일반 트리거입니다(다른 job 들처럼 `workflow_call`
이 아닙니다) — 여기 있는 워크플로 파일 자체를 검사하는 거라 재사용될 필요가
없습니다.
